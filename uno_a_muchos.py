...
def ver_datos_dos():  # Esta función la hice para ver el efecto en las tablas
    ...
    for campos in session.query(Subject).order_by(Subject.id):
        print(campos.id, campos.subject_name, campos.subject_desc, campos.subject_cpe)
    session.close()
    print('')
...
session.add_all({
    ...
    Alumno(code_personal='111979453', firstname='Ana F.', lastname='Caceres O.')})

# Agregar registros con una relacion

# Agregar registros con una relacion
j_alumno = Alumno(code_personal='111979453')
...
for an_a, a_s in session.query(Alumno, Subject). \
        ...
        all():
    print(an_a.lastname, '-', a_s.subject_name)